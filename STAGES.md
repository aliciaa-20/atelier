...
- [ ] 4. **In progress: built, awaiting on-device verification.** Voice sync (`ScriptMatcher`, voice mode in `TeleprompterScroll`/model, `SpeechRecognizer`, mic/speech permissions, top-bar control, "Listening" pill) — built on `feat/teleprompter-voice-sync` (278 tests pass). **On-device checks still open:** recognition quality on a real script, >1 min read (request restart), pill placement/height, permission grant/deny/revoke, VoiceOver, Reduce Motion, AirPods connecting mid-read, CPU ~0% while waiting, mic indicator off when paused
